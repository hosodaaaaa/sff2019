...
gulp.task("watch", done => {
  ...
  gulp.watch(`${config.src.sass}/**/*.@(css|sass)`, gulp.parallel("sass"));
...
// clean
gulp.task("clean", () =>
  del([`${config.dest.html}/**/*.html`, config.dest.css, config.dest.js])
);

// build
gulp.task(
  "build",
  gulp.series("clean", gulp.parallel("pug", "sass", "scripts", "images"))
);
